...
contract LiquidationOperator is IUniswapV2Callee {
    ...
    IUniswapV2Pair immutable uniswapV2Pair_WBTC_WETH; // Pool1
    IUniswapV2Pair immutable uniswapV2Pair_WBTC_USDT; // Pool2

    // contract address of Aave LendingPool that we are going to liquidate
    ILendingPool constant lendingPool =
        ILendingPool(0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9);

    // account address that we are going to liquidate
    ...
    function uniswapV2Call(
        ...
        ) = uniswapV2Pair_WBTC_WETH.getReserves(); // Pool1
        (
            uint256 reserve_WBTC_Pool2,
            uint256 reserve_USDT_Pool2,

        ) = uniswapV2Pair_WBTC_USDT.getReserves(); // Pool2
        ...
        console.log("repay_WBTC(getAmountIn) = %s", repay_WBTC);
        WBTC.transfer(address(uniswapV2Pair_WBTC_USDT), repay_WBTC);
        
        // 2.3 swap WBTC to WETH (since there is no withdraw function in WBTC)
        uint amountWBTC = WBTC.balanceOf(address(this));
        WBTC.transfer(address(uniswapV2Pair_WBTC_WETH), amountWBTC);
